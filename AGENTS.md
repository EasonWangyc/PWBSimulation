...
- `MATERIAL_DB`：材料库路径，默认是 `config/database.mdf`，可通过 `SIM_MATERIAL_DB` 覆盖。
...
- `SOA_DIR`、`SOA_RESULTS_DIR`、`SOA_BASE_FSP`：SOA-PWB-SOA 场景目录、结果目录、基础工程文件。

当前工作区根目录是：

```text
D:/Desktop/WYC files/PWBSimulation
```
...
- `pwb_core.py`：核心模块，包含 SOAPWBParams 参数类、直波导五段几何构建（SOA输出+Taper-1+PWB直段+Taper-2+外置SOA输入）、FDTD 仿真设置、结果读取与可视化、FDE 模式分析（需 MODE Solutions）和模式重叠因子 η 计算。
